...
class FormGeneratorBaseForm(forms.Form):
    ...
    def prepare_upload_file(self, field:Field):
        message = _('The file size is more than limit (limited size: %s bytes)') % field.file_size
    ...
    def __init__(self, form, form_response, *args, **kwargs):
        ...
        for i, field in enumerate(self.instance.get_fields()):
            ...
            if hasattr(self, method):
                ...
                try:
                    self.fields[field_name].initial = form_response_data[i].get('value', None)
                except IndexError:
                    pass

    def prepare_upload_file(self, field:Field):
        widget_attrs: dict = field.build_widget_attrs(self.instance, {'content_type': 'field'})
        field_attrs: dict = field.build_field_attrs({'widget': forms.URLInput(attrs=widget_attrs)})
    ...
    def clean(self):
        ...
        if cleaned_data['genre'] == const.FieldGenre.UPLOAD_FILE:
            if not cleaned_data['file_types']:
                raise forms.ValidationError(_("You should define FileTypes for upload file genre"))
        return cleaned_data
